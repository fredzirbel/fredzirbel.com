        ...
        <div className="rounded-xl border border-line bg-panel/70 p-7">
          ...
          <ul className="mt-4 grid gap-3 sm:grid-cols-2">
            {earnedCertifications.map((cert) => {
              ...
              return <li key={cert.name}>{cert.href ? <a href={cert.href} target="_blank" rel="noopener noreferrer" className={`${cls} hover:border-trace/50 hover:text-trace`}>{content}</a> : <div className={cls}>{content}</div>}</li>;
            })}
          </ul>
        </div>
        <div className="rounded-xl border border-line bg-panel/70 p-7">
        ...
        <div className="grid md:grid-cols-3">
          {capabilityGroups.map((group) => (
            <div key={group.heading} className="border-b border-line p-7 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0">
              <h3 className="font-mono text-xs uppercase tracking-[0.18em] text-trace">{group.heading}</h3>
